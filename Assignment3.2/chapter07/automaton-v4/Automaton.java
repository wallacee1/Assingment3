...
public class Automaton
{
    ...
    public Automaton(int numberOfCells, int wolframCode)
    {
        ...
        stateTable = computeStateTable(wolframCode);
        for(int i : stateTable) {
            System.out.print(i + " ");
        }
        System.out.println();
    ...
    /**
     * Turn wolfram code into a reverse binary number
     * @param wolframCode
     * @return
     */
    private int[] computeStateTable(int wolframCode) 
    {
        int[] stateTable = new int[8];
        wolframCode /= 2;
        for(int i = 1; i < 8; i++) {
            stateTable[i] = wolframCode % 2;
            wolframCode /= 2;
        }
        return stateTable;
    }
